...
for (const [date,frequency,note] of CASES) {
  const r = await api('/scheduled_transactions','POST',{scheduled_transaction:{
    account_id:acct, date, amount:-1000, frequency, memo:`P3-04c ${date} ${frequency}`}}, true)
  console.log(r ? `planted  ${date} ${frequency.padEnd(16)} — ${note}` : `REJECTED ${date} ${frequency}`)
}
